...
const AdminStaffVerification = () => {
  ...
  const handleAccept = async (id) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/admin/staff/${id}/approve`, {
        method: 'PUT',
        headers: {
  ...
  const handleReject = async (id) => {
    try {
      setMessage({ type: '', text: '' });
      const response = await fetch(`${API_BASE_URL}/admin/staff/${id}/reject`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      <div className="flex-1 flex flex-col items-center w-full py-10 px-4">
        ...
        <div className="w-full max-w-3xl flex flex-col gap-8">
          ...
          {!loading && !error && staff.map((s) => (
            <div key={s.id} className="bg-white rounded-xl shadow-lg border border-blue-200 p-6 flex flex-col gap-4">
              <div className="flex flex-wrap gap-6 mb-2">
                ...
                <div className="flex items-center gap-2">
                  <ShieldCheck className={`w-5 h-5 ${s.status === 'accepted' ? 'text-green-600' : s.status === 'rejected' ? 'text-red-600' : 'text-blue-600'}`} />
                  <span className="font-semibold text-gray-700">Status:</span>
                  <span className={`font-bold ${s.status === 'accepted' ? 'text-green-600' : s.status === 'rejected' ? 'text-red-600' : 'text-blue-600'}`}>
